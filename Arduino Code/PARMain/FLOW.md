...
**4. `scanGrid()`** — re-home and re-read every cell with the TCS3200. The 10-min idle disables steppers, so position can drift and discs may have been moved; this reseeds `gridState[]` so `displayBitmap` only flips cells that actually differ. The reading sweep runs with the **flip arm dropped to SCAN (~33.5°, 12.5° below RELEASE)** instead of parked at REST: the sensor trails the flip head by `SCAN_OFFSET_X` (−23 mm ≈ one cell pitch), so the lowered arm brushes the whole board during the serpentine and pushes through any squisk accidentally left at stage 1 (90°, half-rotated). Homing moves still run with the arm at REST (the initial home is before the drop; the mid-scan rehome lifts it first) — a dropped arm dragged diagonally across the populated board is what snapped the flip arm before. The arm is parked back at REST as soon as the scan completes. **Blown-sensor guard:** each cell's clear-channel is checked against `SCAN_C_CEILING` (4000 Hz — no real disc reads that bright at 20% scaling); if `SCAN_OVERBRIGHT_LIMIT` (8) cells exceed it, the front-end has come up in a 2-4× too-bright regime (every cell would misclassify "black" → board reads empty → garbled draw). The sweep bails, `initColorSensor()` re-asserts the S0/S1 scaling in place, and it re-scans (up to `SCAN_REINIT_MAX`=2 times). It does **not** MCU-reset on failure — that would leave the already-popped gallery item drawn-never.
