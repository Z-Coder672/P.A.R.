...
cadence gate → no trusted time-server clock? → HOLD PARKED (no motion, no polling), retry in 30 s
             → night (20:30–09:59), re-checked after EVERY wake? → sleep until 10:00
...
**Daily lidar scan (10:00 local).** `runDailyLidarScan()` sweeps the VL53L4CD over **rows 1–17 (629 cells)** and writes the result to flash; row 0's 37 cells are projected afterwards (see **Skipped top row** below). It runs when the clock says the hour is ≥ 10 **and** the flash record's date isn't today — which covers both "it just turned 10:00" and "we rebooted at 14:00 and owe today's scan". An owed scan is only **started before 18:00** (`LIDAR_SCAN_CUTOFF_HOUR`): at/after the cutoff the day is written off — the rig sleeps to 10:00 and runs the scan on wake — rather than sweeping ~50 min into the evening and then printing into the night. Sensor mechanics are ported from `ScanColorLidarTest`: free-running at a 50 ms timing budget, one fresh 4 s window per cell (~80 samples), reduced to a **20 %-trimmed mean in tenths of a mm**; head offsets `LIDAR_OFFSET_X = +31.075` (calibrated so col 36 targets exactly the `X=0` machine limit) and `LIDAR_OFFSET_Y = SCAN_OFFSET_Y + 6.0 = +14.0` (the top row's target computes to −14.2 + 14 = −0.2, inside GRBL's envelope — this is what the `$131` 406→412 bump bought — but the head *physically* jams there, which is why row 0 is skipped). Targets are clamped on **both** axes (`clampScanY`, and `clampScanX` as the mirror safety net).

**Skipped top row (`LIDAR_SKIP_TOP_ROW`).** The colour sensor sits +8 mm above the flip head and clears row 0 fine, so `scanGrid()` is untouched. The lidar sits +14 mm, and at that height the head **jams against the frame across the right-hand end of row 0** — and since `cellAt` makes row 0 an even (L→R) row, the sweep would *end* the row at col 36, target (`X 0.0`, `Y −0.2`), hard into the corner. A jam does not necessarily raise `ALARM`: the steppers just skip, silently corrupting every later cell until the `y == 8` rehome. So the sweep starts at `LIDAR_FIRST_ROW` (= 1) and row 0 is never visited at the lidar Y. `rowsDone` still counts from row 0, so the resume arithmetic is unchanged.

`projectSkippedTopRow()` then fills row 0 per column, so the stored record stays dense at 666: a least-squares **linear** fit in row index over that column's measured cells (centred at `t = y − 9` for conditioning), evaluated at `y = 0`. **Linear, not cubic, deliberately** — the offline cubic in `ScanColorLidarTest` is fit to a smoothed multi-pass curve, while this one gets a single sweep's raw cells at sd ≈ 2.88 mm per-cell noise, and extrapolating one step past the fitted range amplifies noise with the degree. Measured against the shipped `LIDAR_FIT_CMM` as ground truth (300 noise trials, mean |error| in mm at the documented 2.88 mm noise): **linear 1.37**, quadratic 1.88, hold-nearest-row 2.31, **cubic 2.82** — the cubic is worse than not projecting at all. `LIDAR_PROJ_DEGREE` changes it. The fit is **not** anchored to `LIDAR_FIT_CMM` even though that scores better, because these records are what the offline re-fit consumes to rebuild that table. Guards: a column with under `LIDAR_PROJ_MIN_POINTS` (8) measured cells, a singular solve, or a projection landing outside the column's own measured range ±5 mm falls back to holding the nearest measured row. `topRowProjected` in the flash record flags the synthetic row; `cellsOk` counts only measured cells (629 on a clean sweep).
...
**Flash record.** One fixed-layout `LidarScanRecord` at `/cadence.bin` on the **same LittleFS mount plog uses** — the factory `ffat` partition (`LittleFS.begin(true, "/littlefs", 10, "ffat")`; see `persistent_log.cpp` for why it is not `spiffs`). plog mounts it at boot; `cadenceFsReady()` re-calls `begin()` only as a guard for the case where that mount *failed*, which returns true immediately when the label is already mounted, so plog's files are never touched or reformatted. The struct (**version 2** since 2026-08-25) holds magic / version / grid geometry / `sensorOk` / local `(tm_year, tm_yday)` / unix epoch / OK-cell count / **the `rowsDone` + `(progYear, progYday)` checkpoint** / `topRowProjected` / `uint16_t dist10[666]` / an FNV-1a checksum over everything preceding it. `topRowProjected` reuses what was reserved padding rather than appending a field, so the layout — and therefore the version — is unchanged and no re-scan is forced on upgrade: every record ever written zeroed those bytes, and 0 is exactly right for a record whose top row really was measured. Written temp-then-rename, same crash policy as plog's rotation. Anything unexpected on load — missing, wrong size, wrong magic/version/geometry, bad checksum — is treated as "no scan on record", so the failure direction is *re-scan*, never *skip*. **The stored `(year, yday)` is the authority for "has today's scan run"**, so a reboot at any hour resumes the schedule correctly.
...
Log lines: `ntp start`/`ntp synced`/`ntp NOT synced`, `cadence: record …`, `cadence: lidar scan begin|end`, `cadence: resuming lidar scan at row N/18`, `cadence: record saved … rowsN`, per-cell `ld y<y>c<x> <mm.t> n<samples> mn<min> mx<max>`, `cadence: row 0 lidar SKIPPED (jams at lidar Y; projected after the sweep)`, `cadence: top row projected (deg<N> fit <n>, held <n> of 37 cols)`, `cadence: sleep at HH:MM until 10:00`, `cadence: sleeping HH:MM`, `cadence: wake`, `cadence: no trusted clock - holding parked (no motion, no polling)`.
