...
// stale stop signal can't end a recording started long after the Arduino's
// signal was issued.
...
$id = (is_array($parsed) && isset($parsed['id'])) ? (int)$parsed['id'] : null;

// Pop the flag atomically — caller is responsible for stopping the recording.
@unlink($flag);
echo json_encode(['ok' => true, 'id' => $id, 'entry' => date('c')]);
