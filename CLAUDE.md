...
| `snapshot-request.php` | POST | Arms `snapshot-pending.flag` (auth: `X-Snapshot-Secret`). Accepts optional `id=<N>` form param (digits-only), written as the flag contents; absent/invalid id writes an empty flag (legacy IDless capture). This is the normal snapshot trigger — the Arduino POSTs it with the gallery id right after its check (fix) pass finishes. |
...
- **avfoundation MUST be given `-video_size`.** Requesting `-framerate` alone makes the device config fail and fall back to the cam's default mode; ffmpeg then "can't estimate rate" and hands libx264 a garbage ~5000fps timebase → `MB rate > level limit` → a **grey, corrupted recording** (this was the actual bug). `build_record_cmd` pins `-video_size CAMERA_VIDEO_SIZE` (default `1920x1080`) + `-pixel_format uyvy422`. The benign `Configuration of video device failed, falling back to default` warning may still print even when the size IS honored — verify with `ffprobe` (correct output dims) rather than trusting the warning. Output is forced CFR (`-fps_mode cfr -r <framerate>`) over `-use_wallclock_as_timestamps 1` because the Brio drops its true rate (~15fps in dim light) below the requested 30. Don't re-add `+faststart` — it conflicts with the fragmented-mp4 flags.
